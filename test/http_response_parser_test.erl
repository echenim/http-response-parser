...
%% @doc Splits an HTTP response binary into headers and body sections.
%%
%% This function takes a binary HTTP response and attempts to divide it into
%% the headers part (including the status line) and the body part, using the
%% standard HTTP delimiter "\r\n\r\n". It returns a tuple containing the headers
%% and body binaries if successful. If the delimiter is not found, indicating
%% a malformed HTTP response, it returns an error tuple.
%%
%% @param Binary The complete HTTP response binary.
%% @return {Headers, Body} if the response is properly formatted,
%%         {error, Reason} if the delimiter is missing or any other error occurs.
-spec split_status_and_body(binary()) -> {binary(), binary()} | {error, binary()}.
...
%% parse_headers/1: Parses the binary string containing all HTTP headers into a list of key-value pairs.
%% @param HeadersBinary The binary string of HTTP headers.
%% @return A list of parsed headers as key-value pairs or an error tuple if a malformed header is encountered.
parse_headers(HeadersBinary) ->
    %% Split the headers binary into lines based on the CRLF delimiter.
    HeadersLines = binary:split(HeadersBinary, <<"\r\n">>, [global]),
    try
        %% Attempt to parse each header line into a key-value pair using parse_header/1.
        lists:map(fun parse_header/1, HeadersLines)
    catch
        %% Catch any thrown error indicating a malformed header.
        error:malformed_header -> {error, <<"Malformed header found">>}
    end.

%% parse_header/1: Parses an individual header line into a key-value pair.
%% @param HeaderLine The binary string of a single header line.
%% @return A tuple of {Key, Value} where Key is the header name and Value is its value, both as binaries.
%%         Throws an error if the header line cannot be parsed into a key-value pair.
parse_header(HeaderLine) ->
    case binary:split(HeaderLine, <<": ">>, [global]) of
        %% If the header line is correctly formatted as "Key: Value", parse it.
        [Key, Value] -> {string:to_lower(binary_to_list(Key)), Value};
        %% If the header line does not conform to the expected format, throw an error.
        _ -> throw(malformed_header)
    end.
